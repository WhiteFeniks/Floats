...
int    *ft_division_long_arithmetic(int *x, int *y)
{
    ...
    x = ft_shift_elements_left(x, len + 1);             // смещаем влево наше число 1, чтобы первое число было на 1 разряд больше второго
...
int main()
{
    ...
    c = ft_division_long_arithmetic (a, b);                               // запуск функции деления длинной арифметики
